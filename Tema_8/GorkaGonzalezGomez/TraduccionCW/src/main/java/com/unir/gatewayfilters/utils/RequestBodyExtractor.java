package com.unir.gatewayfilters.utils;
 ...
 * It also sets the headers of the request, removing the Content-Length header and setting the Transfer-Encofing header to "chunked".
 */
@Component
...
public class RequestBodyExtractor {
     ...
     *  It first retains the DataBuffer, then splits it into a Flux of DataBufffers.
     *  It then converts the Flux of Databuffers into a raw request string.
     *  The raw request string is the converted into a GatewayRequest object using the ObjectMapper.
     *  The headers of the rquest are then set, removing the Content-Length header and setting the Transfer-Encoding header to "chunked".
