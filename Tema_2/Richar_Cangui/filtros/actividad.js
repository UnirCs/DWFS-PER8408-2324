...
function blackAndWhiteConverter() {
    ...
    pixels = pixels.map((fila) => {
        return fila.map((pixel) => {
            let bin = (pixel[0] + pixel[1] + pixel[2]) / 3 < 128 ? 0 : 255;
            return [bin, bin, bin];
        });
    });
...
function scaleDown() {
    ...
    let pixels = handler.getPixels();
    let downPixels = [];
    pixels.forEach((fila, index) => {
        if (index % 2 !== 0) return;
        let newFila = [];
        fila.forEach((pixel, index) => {
            if (index % 2 !== 0) return;
            newFila.push(pixel);
        });
        downPixels.push(newFila);


    });
    console.log("PIXELS: ");
    console.log(downPixels);
    handler.savePixels(
        downPixels,
        outputPath,
...
function dimBrightness(dimFactor) {
    ...
    pixels = pixels.map((fila) => {
        return fila.map((pixel) => {
            return [pixel[0] / dimFactor, pixel[1] / dimFactor, pixel[2] / dimFactor];
        });
    });
