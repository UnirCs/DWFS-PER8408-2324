...
function redConverter() {
    ...
    for (let x = 0; x < pixels.length; x++){
        pixels[x].map(y=>y[1]=0);
        pixels[x].map(y=>y[2]=0);
    }
...
function greyConverter() {
    ...
    for (let x = 0; x < pixels.length; x++){
        pixels[x]=pixels[x].map(function (arg){
            let media = (arg[0]+arg[1]+arg[2])/3;
            arg[0]=media;
            arg[1]=media;
            arg[2]=media;
            return arg;
...
function blackAndWhiteConverter() {
    ...
    for (let x = 0; x < pixels.length; x++){
        pixels[x]=pixels[x].map(function (arr){
...
function dimBrightness(dimFactor) {
    ...
    for (let x = 0; x < pixels.length; x++){
        pixels[x]=pixels[x].map(y=>y.map(x=>x/dimFactor));
    }
...
function invertColors() {
    ...
    for (let x = 0; x < pixels.length; x++){
        pixels[x]=pixels[x].map(y=>y.map(x=>255-x));
    }
...
function merge(alphaFirst, alphaSecond) {
    ...
    let pixels = [];
    catPixels=catPixels.map(x=>x.map(y=>y.map(z=>z*alphaFirst)));
    dogPixels=dogPixels.map(x=>x.map(y=>y.map(z=>z*alphaSecond)));
    pixels=catPixels;
    let contador=0;
    for(let x=0; x < pixels.length;x++){
        contador=0;
        pixels[x]=pixels[x].map(function (arg){
            arg[0]=arg[0]+dogPixels[x][contador][0];
            arg[1]=arg[1]+dogPixels[x][contador][1];
            arg[2]=arg[2]+dogPixels[x][contador][2];
            contador++;
            return arg;
        });
    }
