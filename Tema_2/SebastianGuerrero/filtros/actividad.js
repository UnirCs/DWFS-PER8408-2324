...
function redConverter() {
    ...
    //Aqui tu codigo
    for(let filas of pixels){
        for(let rgb of filas){
            rgb[1]=rgb[2]=0;
        }
    }
...
function greyConverter() {
    ...
    for(let filas of pixels){
        for(let rgb of filas){
            let valor = (rgb[0]+rgb[1]+rgb[2])/3;
            rgb[0]=rgb[1]=rgb[2]=valor;
        }
    }
    handler.savePixels(pixels, outputPath);
}
...
function scaleDown() {
    ...
    for(let i = 0; i < pixels.length  ; i += 2){
        ...
        for(let j =0; j < pixels[i].length  ; j += 2){
            newFila.push(pixels[i][j]);
        }
        newPixels.push(newFila);
    }
    ...
    for(let i= 0; i< catPixels.length; i++){
        for(let j= 0; j < catPixels[i].length; j++){
            for(let k= 0; k < catPixels[i][j].length; k++){
                pixels[i][j][k]= catPixels[i][j][k]*alphaFirst +dogPixels[i][j][k]*alphaSecond;
            }
        }
    }
