...
function redConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            pixels[i][j][1] = 0; // Establecer el canal G a 0
            pixels[i][j][2] = 0; // Establecer el canal B a 0
        }
    }
...
function greenConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            pixels[i][j][0] = 0; // Establecer el canal R a 0
            pixels[i][j][2] = 0; // Establecer el canal B a 0
        }
    }
...
function blackAndWhiteConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            ...
            if (average < 128) {
                pixels[i][j] = [0, 0, 0]; // Establecer el píxel en negro
            } else {
                pixels[i][j] = [255, 255, 255]; // Establecer el píxel en blanco
            }
        }
    }
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let i = 0; i < catPixels.length; i++) {
        ...
        for (let j = 0; j < catPixels[i].length; j++) {
            let mergedPixel = [
                Math.round(alphaFirst * dogPixels[i][j][0] + alphaSecond * catPixels[i][j][0]),
                Math.round(alphaFirst * dogPixels[i][j][1] + alphaSecond * catPixels[i][j][1]),
                Math.round(alphaFirst * dogPixels[i][j][2] + alphaSecond * catPixels[i][j][2])
            ];
