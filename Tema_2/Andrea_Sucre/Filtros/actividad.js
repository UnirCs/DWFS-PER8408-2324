...
function greyConverter() {
    ...
    for (let rowCounter = 0; rowCounter < pixels.length; rowCounter++) {
        let row = pixels[rowCounter];
        for (let columnCounter = 0; columnCounter < row.length; columnCounter++) {
            const mean = row[columnCounter].reduce((total, value) => total + value) / row[columnCounter].length;
            row[columnCounter] = row[columnCounter].map(value => value = mean);
        }
    }
...
function blackAndWhiteConverter() {
    ...
    for (let rowCounter = 0; rowCounter < pixels.length; rowCounter++) {
        ...
        for (let columnCounter = 0; columnCounter < row.length; columnCounter++) {
            const mean = row[columnCounter].reduce((total, value) => total + value) / row[columnCounter].length;
            if (mean < 128) {
                row[columnCounter] = row[columnCounter].map(value => value = 0);
            } else {
                row[columnCounter] = row[columnCounter].map(value => value = 255);
            }
        }
    }
...
function dimBrightness(dimFactor) {
    ...
    for (let rowCounter = 0; rowCounter < pixels.length; rowCounter++) {
        let row = pixels[rowCounter];
        for (let columnCounter = 0; columnCounter < row.length; columnCounter++) {
            row[columnCounter] = row[columnCounter].map(value => value = value / dimFactor);
        }
    }
...
function invertColors() {
    ...
    for (let rowCounter = 0; rowCounter < pixels.length; rowCounter++) {
        let row = pixels[rowCounter];
        for (let columnCounter = 0; columnCounter < row.length; columnCounter++) {
            row[columnCounter] = row[columnCounter].map(value => value = 255 - value);
        }
    }
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let rowCounter = 0; rowCounter < catPixels.length; rowCounter++) {
        pixels[rowCounter] = [];
        for (let columnCounter = 0; columnCounter < catPixels[rowCounter].length; columnCounter++) {
            pixels[rowCounter][columnCounter] = catPixels[rowCounter][columnCounter].map((pixel, index) => pixel * alphaSecond + dogPixels[rowCounter][columnCounter][index] * alphaFirst);
        }
    }
