...
function ejemplo() {
    ...
    let outputPath = '../../00_Resources/filtros/output/ejemplo.jpg';
    let pixeles = [];
    let filas = 2;
    let columnas = 2;
...
function redConverter() {
    let outputPath = '../../00_Resources/filtros/output/tucan_red.jpg';
...
function greenConverter() {
    let outputPath = '../../00_Resources/filtros/output/tucan_green.jpg';
    let pixels = handler.getPixels();
...
function blueConverter() {
    let outputPath = '../../00_Resources/filtros/output/tucan_blue.jpg';
    let pixels = handler.getPixels();
...
function greyConverter() {
    let outputPath = '../../00_Resources/filtros/output/tucan_grey.jpg';
    let pixels = handler.getPixels();

    //Aqui tu codigo
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            let total = pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2]
...
function blackAndWhiteConverter() {
    let outputPath = '../../00_Resources/filtros/output/tucan_black_and_white.jpg';
    let pixels = handler.getPixels();
 ...
 */
function scaleDown() {
    let outputPath = '../../00_Resources/filtros/output/tucan_scale_down.jpg';
    let pixels = handler.getPixels();
    let newPixels = [];
...
function dimBrightness(dimFactor) {
    let outputPath = '../../00_Resources/filtros/output/tucan_dimed.jpg';
    let pixels = handler.getPixels();
...
function invertColors() {
    let outputPath = '../../00_Resources/filtros/output/tucan_inverse.jpg';
    let pixels = handler.getPixels();
...
function merge(alphaFirst, alphaSecond) {
    ...
    let outputPath = '../../00_Resources/filtros/output/merged.jpg';
    ...
    for (let i = 0; i < dogPixels.length; i++) {
        ...
        for (let j = 0; j < dogPixels[i].length; j++) {
            let channels = []
            for (let k = 0; k < dogPixels[i][j].length; k++) {
                channels.push(alphaFirst*dogPixels[i][j][k] + alphaSecond*catPixels[i][j][k])
            }
