...
function greyConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            let sumaTotal =0;
            //Primer bucle para sumar
            for(let k=0;k< pixels[i][j].length;k++){
                sumaTotal+= pixels[i][j][k];
            }
            let media= sumaTotal/pixels[i][j].length;
            //Segundo bloque para añadir el gris
            for(let k=0;k< pixels[i][j].length;k++){
                pixels[i][j][k]=media;
            }
        }
...
function blackAndWhiteConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            ...
            let media= sumaTotal/pixels[i][j].length;
            let valorPixels=255;
            //Si la media no es suficiente, el valor que le metemos es 0
            if (media<128) {
                valorPixels=0;
            }
            //Segundo bloque para añadir el gris
            for(let k=0;k< pixels[i][j].length;k++){
                pixels[i][j][k]=valorPixels;
            }
        }
    }
    handler.savePixels(pixels, outputPath);
...
function scaleDown() {
    ...
    //Aqui tu codigo
    //Primero vamos a cortar las filas pares
    let pixelReducidos = []
    for (let i=0; i<pixels.length;i=i+2)
    {
        let filaNova = [];
        for (let j=0; j<pixels[i].length; j=j+2){
            filaNova.push(pixels[i][j]);
        }
        pixelReducidos.push(filaNova);
    }
    pixels = pixelReducidos;
    ...
    for (let i = 0; i < dogPixels.length; i++) {
        for (let j = 0; j < dogPixels[i].length; j++) {
            for(let k=0;k< dogPixels[i][j].length;k++){
                pixels[i][j][k] = catPixels[i][j][k]*alphaSecond + dogPixels[i][j][k]*alphaFirst ;
            }
        }
    }
