...
function greyConverter() {
    ...
    for(let i = 0; i < pixels.length; i++){
        for(let j = 0; j < pixels[i].length; j++){
            const escalaGrises = calcularEscalaGrises(pixels[i][j][0], pixels[i][j][1], pixels[i][j][2])
            pixels[i][j][0] = escalaGrises;
            pixels[i][j][1] = escalaGrises;
            pixels[i][j][2] = escalaGrises;
        }
    }
...
function calcularEscalaGrises(r, g, b) {
    return (r + g + b)/3;
}
...
function blackAndWhiteConverter() {
    ...
    for(let i = 0; i < pixels.length; i++){
        for(let j = 0; j < pixels[i].length; j++){
            pixels[i][j] = calcularBlancoYNegro(pixels[i][j][0], pixels[i][j][1], pixels[i][j][2])
        }
    }
...
function calcularBlancoYNegro(r, g, b) {
    const media = (r + g + b)/3;
    if(media > 128){
        return [255, 255, 255];
    }
...
function scaleDown() {
    ...
    let pixels = handler.getPixels();
    const pixelsScaledDown = [];

    for (let i = 0; i < pixels.length; i += 2) {
        const filaNueva = [];
        for (let j = 0; j < pixels[i].length; j += 2) {
            filaNueva.push(pixels[i][j]);
        }
        pixelsScaledDown.push(filaNueva);
    }
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let i = 0; i < dogPixels.length; i++) {
        ...
        for (let j = 0; j < dogPixels[i].length; j++) {
            let mergedPixel = [
                alphaFirst * dogPixels[i][j][0] + alphaSecond * catPixels[i][j][0],
                alphaFirst * dogPixels[i][j][1] + alphaSecond * catPixels[i][j][1],
                alphaFirst * dogPixels[i][j][2] + alphaSecond * catPixels[i][j][2]
            ];
