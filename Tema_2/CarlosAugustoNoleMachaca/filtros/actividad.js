    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            for (let k = 0; k < pixels[i][j].length; k++) {
                sumaPixel += pixels[i][j][k];
            }
            mediaPixel = sumaPixel / 3;
...
function scaleDown() {
    ...
    let newImageScaleDown = [];
    for (let i = 0; i < pixels.length; i++) {
        if (i % 2 == 0) {
            newImageScaleDown.push([]);
            for (let j = 0; j < pixels[i].length; j++) {
                if (j % 2 === 0) {
                    newImageScaleDown[newImageScaleDown.length - 1].push(pixels[i][j]);
                }
            }
        }
    }

    handler.savePixels(newImageScaleDown, outputPath, handler.getShape()[0] / 2, handler.getShape()[1] / 2);
}
...
function dimBrightness(dimFactor) {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            for (let k = 0; k < pixels[i][j].length; k++) {
                newPixel = pixels[i][j][k] / dimFactor;
                pixels[i][j][k] = newPixel;
            }
            newPixel = 0;
        }
    }
