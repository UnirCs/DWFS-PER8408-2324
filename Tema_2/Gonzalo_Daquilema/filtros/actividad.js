...
function scaleDown() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        if (i % 2 === 0) {
            ...
            pixelsReturn.push(row);
        }
    }


    handler.savePixels(pixelsReturn, outputPath, handler.getShape()[0] / 2, handler.getShape()[1] / 2);
}
...
function merge(alphaFirst, alphaSecond) {
    ...
    for (let i = 0; i < dogPixels.length; i++) {
        ...
        for (let j = 0; j < dogPixels[i].length; j++) {
            dogPixels[i][j][0] *=  alphaFirst;
            dogPixels[i][j][1] *=  alphaFirst;
            dogPixels[i][j][2] *=  alphaFirst;
            catPixels[i][j][0] *=  alphaSecond;
            catPixels[i][j][1] *=  alphaSecond;
            catPixels[i][j][2] *=  alphaSecond;

            let pixelR=dogPixels[i][j][0] + catPixels[i][j][0]
            let pixelG=dogPixels[i][j][1] + catPixels[i][j][1]
            let pixelB=dogPixels[i][j][2] + catPixels[i][j][2]

            row.push([pixelR, pixelG, pixelB]);
        }
        pixels.push(row);
    }
