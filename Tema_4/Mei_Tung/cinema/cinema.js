...
const classInicial = "btn-outline-light";
const classOcupado = "btn-danger";
const classSeleccionado = "btn-light";
...
function setup() {
    ...
    for (let i = 0; i < N; i++) {
        ...
        for (let j = 0; j < N; j++) {
            // Asignar id
            document.getElementsByClassName(classInicial)[idContador - 1].id = `btn${idContador}`;
            // Nuevo asiento
            fila.push({
                id: idContador++,
    ...
    // deshabilitar el último asiento de la última fila
    let asiento = butacas[N - 1][N - 1];
    asiento.estado = true;
    let button = document.getElementsByClassName(classInicial)[asiento.id - 1];
    button.classList.remove(classInicial);
    button.classList.add(classOcupado);
...
function suggest(num) {
    ...
    // proceder si el número de asientos es mayor que 0 y menor o igual a la cantidad de asientos de una fila
    if (num > 0 && num <= N) {
...
function clear() {
    Array.from(document.getElementsByClassName(classSeleccionado)).forEach(asiento => {
        asiento.classList.remove(classSeleccionado);
        asiento.classList.add(classInicial);
...
document.getElementById('num').addEventListener('change', (event) => {
    clear();
    suggest(event.target.value).forEach(id => {
        let asiento = document.getElementById(`btn${id}`);
        asiento.classList.remove(classInicial);
        asiento.classList.add(classSeleccionado);
